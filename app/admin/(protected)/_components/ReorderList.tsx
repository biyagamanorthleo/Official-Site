...
export default function ReorderList({ initialItems, orderField = 'sort_order' }: Props) {
  ...
  async function saveOrder() {
    ...
    await Promise.all(
      items.map((item, i) =>
        supabase.from(table).update({ [orderField]: i + 1 }).eq('id', item.id)
      )
    );
    setSaving(false);
    router.refresh();
  ...
  async function handleDelete(item: ReorderItem) {
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <div
            ...
            className={`flex items-center cursor-grab active:cursor-grabbing rounded-2xl border transition-all duration-150 select-none overflow-hidden ${
              dragOver === item.id
          ...
          >
            ...
            <div className="flex-1 px-5 py-4 min-w-0">
              ...
              {item.subtitle && (
                <span className="text-[9px] font-black uppercase tracking-widest text-red-800 mt-0.5 block truncate">
                  {item.subtitle}
                </span>
              )}
            </div>
