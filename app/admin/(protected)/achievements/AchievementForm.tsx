...
export default function AchievementForm({ achievement }: { achievement?: Achievement }) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6 max-w-2xl">
      ...
      <div>
        <label className="text-[10px] font-black uppercase tracking-widest text-ink-muted block mb-3">Image</label>
        <ImageUpload value={form.image} onChange={url => set('image', url)} folder="achievements" />
      </div>

      <div>
        <label className="text-[10px] font-black uppercase tracking-widest text-ink-muted block mb-2">Year</label>
      ...
      <div>
        <label className="text-[10px] font-black uppercase tracking-widest text-ink-muted block mb-2">Description</label>
      ...
      <div className="flex gap-4 pt-4">
        ...
        <button type="button" onClick={() => router.back()} className="px-8 py-4 rounded-xl text-ink-muted text-xs font-black uppercase tracking-widest bg-white/5 hover:bg-white/10 transition-all">Cancel</button>
      </div>
    </form>
  );
}
