    ...
    <div className="max-w-5xl">
      <div className="flex items-start justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-ink-muted text-xs font-bold uppercase tracking-widest mt-1">Review member posts &amp; publish club posts</p>
        </div>
      ...
      <section className="mb-14">
        ...
        {!pending?.length ? (
          <div className="py-12 text-center border border-dashed border-white/5 rounded-2xl">
            <p className="text-ink-muted text-xs font-black uppercase tracking-widest">All clear — no pending posts</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mb-3">
                      <span className="flex items-center gap-1 text-ink-muted text-[10px] font-bold uppercase tracking-widest">
                    ...
                    {post.excerpt && (
                      <p className="text-ink-muted text-sm leading-relaxed line-clamp-2">{post.excerpt}</p>
                    )}
                    {post.content && (
                      <p className="text-ink-muted text-xs leading-relaxed mt-2 line-clamp-3">{post.content}</p>
                    )}
                  </div>
                  {post.cover_image && (
      ...
      {!!recent?.length && (
        <section>
          <h2 className="text-[10px] font-black uppercase tracking-widest text-ink-muted mb-4">Recently Reviewed</h2>
          <div className="space-y-2">
            {recent.map(post => (
              <div key={post.id} className="flex items-center gap-4 bg-[#050505] border border-white/5 rounded-xl px-5 py-4">
                ...
                <span className="text-ink-muted text-[10px] font-bold uppercase tracking-widest hidden sm:block">{post.author}</span>
                {post.rejection_note && (
                ...
                <Link href={`/admin/blog/${post.id}`}
                  className="text-ink-muted text-[10px] font-black uppercase tracking-widest hover:text-white border border-white/5 hover:border-white/15 rounded-lg px-3 py-1.5 transition-all flex-shrink-0">
                  Edit
                </Link>
                <DeletePostButton id={post.id} />
              </div>
