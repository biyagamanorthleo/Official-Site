...
function MemberCard({ member, size = 'md' }: { member: Member; size?: 'lg' | 'md' }) {
  return (
    <div className={`group ${size === 'lg' ? 'md:col-span-2' : ''}`}>
      <div className={`img-skeleton relative ${size === 'lg' ? 'aspect-[3/2]' : 'aspect-square'} rounded-2xl overflow-hidden mb-4 border border-white/5 bg-[#0a0a0a] group-hover:border-red-500/30 transition-all duration-700`}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={member.photo} alt={member.name} loading="lazy" className="absolute inset-0 w-full h-full object-cover grayscale opacity-50 transition-all duration-700 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-105" />
